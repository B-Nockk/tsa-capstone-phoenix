...
ans-ping: ans-check ## Ping all nodes
	@cd $(ANSIBLE_DIR) && ansible -i inventory/$(ENV)/hosts.ini -m ping all --user=$(ANSIBLE_USER) --ssh-extra-args='$(ANSIBLE_SSH_ARGS)'

.PHONY: ans-run
ans-run: ans-check ## Run main playbook
	@cd $(ANSIBLE_DIR) && ansible-playbook -i inventory/$(ENV)/hosts.ini playbooks/site.yml --user=$(ANSIBLE_USER) --ssh-extra-args='$(ANSIBLE_SSH_ARGS)'

.PHONY: ans-server-only
ans-server-only: ans-check ## Run server-only playbook
	@cd $(ANSIBLE_DIR) && ansible-playbook -i inventory/$(ENV)/hosts.ini playbooks/k3s-server.yml --user=$(ANSIBLE_USER) --ssh-extra-args='$(ANSIBLE_SSH_ARGS)'

.PHONY: ans-agent-only
ans-agent-only: ans-check ## Run agent-only playbook
	@cd $(ANSIBLE_DIR) && ansible-playbook -i inventory/$(ENV)/hosts.ini playbooks/k3s-agent.yml --user=$(ANSIBLE_USER) --ssh-extra-args='$(ANSIBLE_SSH_ARGS)'
